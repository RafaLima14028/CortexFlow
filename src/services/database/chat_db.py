...
async def add_new_message(
    content: str, role: str, user_id: str, chat_id: str, db: AsyncIOMotorDatabase
) -> None:
    document = {
        "content": content,
        "role": role,
    ...
    await db["chat_history"].insert_one(document=document)

    return


async def get_all_id_by_user_id(
    user_id: str, db: AsyncIOMotorDatabase, limit: int = 10, skip: int = 0
) -> list[str]:
    chat_ids_list = (
        await db["agno_sessions"]
        .aggregate(
            [
                ...
                {"$skip": skip},
                {"$limit": limit},
                {"$project": {"_id": 0, "chat_id": "$_id"}},
            ]
        )
        .to_list(length=limit)
    )

    return [str(doc["chat_id"]) for doc in chat_ids_list]
